import config from './gameConfigLocal.js';
...
export function setPlayerInput(playerId, input) {
...
export function addPlayer(id, nickname) {
...
// Helper per sbloccare gli ostacoli direttamente (porta)
export function unlockObstacle(targetId) {
  const obs = config.arena.obstacles.find(o => o.id === targetId);
  if (obs) {
    config.arena.obstacles = config.arena.obstacles.filter(o => o.id !== targetId);
    return obs;
  }
  return null;
}
